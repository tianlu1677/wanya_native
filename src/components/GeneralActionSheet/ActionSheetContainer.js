...
export default class extends React.PureComponent {
  ...
  _renderTitle = (title, message) => {
    ...
    return (
      <View style={styles.title} key={'title'}>
        ...
        {message && <Text style={[style, messageStyle]}>{message}</Text>}
      </View>
    );
  };
  ...
  _renderItem = (item, index) => {
    ...
    const textStyle = isCancel ? cancelButtonStyle : isDestructive ? destructiveButtonStyle : null;
    return (
      <TouchableHighlight
        key={index}
        style={styles.button}
        ...
        <View style={styles.buttonView}>
          <Text style={[{fontSize, color}, textStyle]}>{item}</Text>
        </View>
      </TouchableHighlight>
    );
  };
  ...
  _renderSection(items, index, style) {
    ...
    const props = isArray
      ...
      : {};
    return (
      <Component key={index} style={[styles.section, style, {backgroundColor}]} {...props}>
        {isArray
