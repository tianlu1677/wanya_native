...
const openUrl = ({startLocation, destLocation, mode, type, appName}) => {
  ...
  const GaoDeT = mode === 'drive' ? '0' : mode === 'bus' ? '1' : mode === 'walk' ? '2' : '3'; // 导航模式: t = 0（驾车）= 1（公交）= 2（步行）= 3（骑行）= 4（火车）= 5（长途客车）
  ...
  // 腾讯地图参数配置
  const TengXunType =
    mode === 'drive' ? 'drive' : mode === 'bus' ? 'bus' : mode === 'ride' ? 'bike' : 'walk'; // 路线规划方式参数: 公交：bus 驾车：drive 步行：walk 骑行：bike
      ...
      [
        ...
        `qqmap://map/routeplan?type=${TengXunType}&from=${
          startLocation.title
            ? startLocation.title + '&fromcoord=' + startLocation.lat + ',' + startLocation.lng
            : '&fromcoord=CurrentLocation'
        }&to=${destLocation.title}&tocoord=${destLocation.lat},${
          destLocation.lng
        }&referer=${tmapKey}`,
      ],
    ];
