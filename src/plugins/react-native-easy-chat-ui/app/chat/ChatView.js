...
class ChatWindow extends PureComponent {
    ...
    if (code === '/{del}') {
      ...
      if (this.state.cursorIndex < currentTextLength) {
        ...
        if (lastChar === ']') {
          ...
          if (castArray) {
            const cast = castArray[castArray.length - 1];
            lastText = this.state.messageContent.substring(
              0,
              this.state.messageContent.length - cast.length
            );
      ...
      if (currentTextLength >= this.state.cursorIndex) {
        ...
        const nextTemp = this.state.messageContent.substring(
  ...
  renderBg = bg => {
    ...
    if (renderChatBg === undefined) {
      ...
      return (
        <ImageComponent
          source={source}
          style={{position: 'absolute', width, top: 0, height}}
          resizeMode="cover"
        />
      );
    } else {
  ...
  render() {
    ...
    const panelContainerHeight =
      allPanelHeight + (this.isIphoneX ? this.props.iphoneXBottomPadding : 0);
    return (
      <View
        style={{
        ...
        <Animated.View
          ...
          />
          {voiceCustom ? null : this.state.showVoice ? (
            <Voice
              ImageComponent={ImageComponent}
              ref={e => (this.voice = e)}
