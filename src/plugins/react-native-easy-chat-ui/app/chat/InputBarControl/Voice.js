...
class Voice extends PureComponent {
  _renderContent = () => {
    ...
    if (showVoice) {
      ...
      return (
        voiceIcon || (
          <ImageComponent
            source={require('../../source/image/voice.png')}
            style={{width: 30, height: 30}}
          />
        )
      );
  ...
  render() {
    const {inputHeightFix, onMethodChange} = this.props;
    return (
      <View
        style={{height: 35 + inputHeightFix, justifyContent: 'center', alignItems: 'center'}}
        activeOpacity={0.7}>
