...
class VoiceButton extends PureComponent {
  ...
  _onPan = e => {
    ...
    if (!audioHasPermission && Platform.OS === 'android') {
      return null;
    }
  ...
  render() {
    ...
    return (
      <PanGestureHandler
        onGestureEvent={e => this._onPan(e.nativeEvent)}
        onHandlerStateChange={e => this._onPanState(e.nativeEvent)}>
        <TapGestureHandler onHandlerStateChange={e => this._onTab(e.nativeEvent)}>
          <View style={{borderRadius: 18, backgroundColor: isVoiceEnd ? '#000' : '#fff'}}>
            <View style={[styles.container, {height: 35 + inputHeightFix}]}>
              <Text style={[styles.text, {color: isVoiceEnd ? '#fff' : '#000'}]}>
